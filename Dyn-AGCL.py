...
def evaluate_model(model, test_loader):
    ...
    with torch.no_grad():
        for user_embedding, label in test_loader:
            prediction = model(user_embedding)
    ...
    test_rmse = mean_squared_error(all_labels, all_predictions)
...
def main():
    ...
    drop_param = 1.0  # Example value, adjust based on your needs
    gamma = 0.5       # Example value, adjust based on your needs
    ...
    train_data, test_data = split_and_save_data(binary_ground_truth)
    ...
    user_embeddings = torch.FloatTensor(fused_embedding[:len(unique_users)])
    model.eval()
    with torch.no_grad():
        for user_idx, user_embedding in enumerate(user_embeddings):
            user_embedding = user_embedding.unsqueeze(0)
            user_scores = model(user_embedding).numpy().flatten()
            top_k_indices = np.argsort(user_scores)[-5:][::-1]
